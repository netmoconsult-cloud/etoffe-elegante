        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="bg-white border rounded-xl p-6">
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <input type="text" value={name} onChange={(e) => setName(e.target.value)} className="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-1 focus:ring-black" placeholder="Votre nom" />
                </div>
                <div>
                  <label className="block text-sm font-medium mb-1">Adresse de livraison *</label>
                  <textarea value={address} onChange={(e) => setAddress(e.target.value)} rows={3} className="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-1 focus:ring-black" placeholder="Votre adresse complète (Dakar, Sicap Liberté, ...)" />
                  <p className="text-xs text-neutral-400 mt-1">📍 Livraison à Dakar et banlieue uniquement</p>
                </div>
                <div>
                  <label className="block text-sm font-medium mb-1">Numéro de téléphone *</label>
                  <input type="tel" value={phone} onChange={(e) => setPhone(e.target.value)} className="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-1 focus:ring-black" placeholder="+221 77 000 00 00" />
                </div>
              </div>
            </div>
          </div>
          ...
          <div>
            <div className="bg-neutral-50 rounded-xl p-6 sticky top-24">
              ...
              <button onClick={handleOrder} disabled={loading} className="w-full mt-6 bg-black text-white py-3 rounded-full hover:opacity-80 transition flex items-center justify-center gap-2">
                <Heart size={18} /> {loading ? "Préparation..." : "Confirmer la commande"}
              </button>
              
              <p className="text-xs text-neutral-400 text-center mt-4">📱 La facture vous sera envoyée après confirmation</p>
            </div>
          </div>
        </div>
      </div>
